    ...
    private: //Heper Functions
        ...
        void bubbleUp(int index) {
            ...
            while (curr && curr->prev) {
                ...
                swapNodesByPointer(prevNode, curr);
                // curr is same pointer, now it has a new prev; loop continues
            }
        }
        ...
        void bubbleDown(int index) {
            ...
            while (curr) {
                Node* c1 = curr->next;
                Node* c2 = (c1 ? c1->next : nullptr);
                if (!c1) break;

                Node* best = c1;
                if (c2 && !inCorrectOrder(c1->value, c2->value)) best = c2;

                if (!inCorrectOrder(curr->value, best->value)) {
        ...
        //Redorder the entire list after construction
        void reorderList(){
            //Start from middle toward the head
